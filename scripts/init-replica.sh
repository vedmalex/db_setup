...
# Проверяем доступность вторичного сервера MongoDB
if ! check_mongodb_available "mongo2" "$MAX_RETRIES" "$RETRY_INTERVAL"; then
    echo "Error: Secondary MongoDB server is not available"
    exit 1
fi
